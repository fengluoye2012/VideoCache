...
public class MyGSYVideoManager extends GSYVideoBaseManager {
    ...
    private MyGSYVideoManager() {
        init();
        setOptionModelList(OptionUtil.getInstance().getOptionList());
    }
     ...
     * 同步创建一个临时管理器
     */
    public static synchronized MyGSYVideoManager tmpInstance(GSYMediaPlayerListener listener) {
        MyGSYVideoManager gsyVideoManager = new MyGSYVideoManager();
     ...
     * 替换管理器
     */
    public static synchronized void changeManager(MyGSYVideoManager gsyVideoManager) {
